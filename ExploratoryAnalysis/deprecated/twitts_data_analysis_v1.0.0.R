...
root_path <- "C:/Users/DrosoNeuro/Dropbox/UZH_Master/Masterarbeit/Journal/TwitterData/tweets_from_todd" # defining root_path containing all relevant documents
...
# 
#   setwd(paste(root_path) # setting WD
...
#   setwd(paste(root_path,"/csv_files/sick_csv")) # temporarily set WD to folder with files from healthy Twitter users
#     
...
#   setwd(paste(root_path,"/csv_files/one_hundred_csv") # temporarily set WD to folder with files from healthy Twitter users
#   temp = list.files(pattern="*.csv") #read names of all .csv files
# 
#   #creates names from csv-files in folder;
#   names <- setNames(temp, make.names(gsub("*.csv$", "", temp))) #gsub uses regex to replace the specified patterns within a name
...
#   
  ...
  coord_selection  <- function(dataframe,coord_selec) #funtion to make selection of dataframe based on coordinate (lon_west,lon_est,lat_south,lat_north)
  {
...
# tweets on maps using scatterplots----
  ...
  # %%%%%% get only sick tweets!! %%%%%%
    plot_location <- function(dataframe,explore,tag)
    {
      ...
      tot_sick <- dataframe[dataframe[,5]==1,] #subset of all tweets labelled as "sick"
      
      #png(filename=filenames[1],width=2000,height=2000) #open pdf to save images
      p <- print_map(my_ggmap,my_title="Sick tweets",my_coord=tot_sick,colo="red",2)
      p <- print
      ggsave(file=filenames[1],width=img_size,height=img_size)
      #dev.off() #close pdf
      
      remove(list=("p","tot_sick")) #remove map created and dataset to save memory
      
      ##print all tweets labelled as "healthy" in the whole USA##
      tot_healthy <- dataframe[dataframe[,5]==0,]
      
      p <- print_map(my_ggmap,my_title="Healthy tweets",my_coord=tot_healthy,colo="blue",pt_size=2)
      ggsave(file=filenames[3],width=img_size,height=img_size)
      
      remove(list=("p","tot_healthy")) #remove map created and dataset to save memory
      
      ##print all tweets mislabelled in the whole USA##
      tot_mislabelled <- dataframe[which(dataframe[,1] %in% explore$false_label),] #gets all tweets from those users who are in the wrong category ("sick" users in the "healthy" dataset or "healthy" users in the "sick" data set)
      p <- print_map(my_ggmap,my_title="Mislabelled Tweets",my_coord=tot_mislabelled,col="deeppink",pt_size=2)
      ggsave(file=filenames[5],width=img_size,height=img_size)
      
      remove(list=("p","tot_mislabelled","my_ggmap")) #remove map created and dataset to save memory; also remove ggmap of USA since it won't be used afterwards
      ...
      local_selection <- dataframe[which(dataframe[,"latitude",]>=coord_local[1] & dataframe[,"latitude"] <= coord_local[3] & dataframe[,"longitude"] >= coord_local[2] & dataframe[,"longitude"] <= coord_local[4]),] #select only those values which are within the local map region
            
      ##print all tweets labelled as "sick" in the selected region##
      local_sick <- local_selection[local_selection[,5]==1,] #get all local tweets labelled as "sick"
      #png(filename=filenames[2],width=2000,height=2000) 
      p <- print_map(my_ggmap_local,my_title = "Sick tweets", local_sick,"red",pt_size=10)
      ggsave(file=filenames[2],width=img_size,height=img_size)
      #dev.off() #close pdf
      
      remove(list=("p","local_sick")) #remove map created and dataset to save memory;
      
      #print all tweets labelled as "healthy" in the selected region##
      local_healthy <- local_selection[local_selection[,5]==0,]
          
      p <-print_map(my_ggmap_local,my_title="Healthy tweets",local_healthy,colo="blue",pt_size=10)
      ggsave(file=filenames[4],width=img_size,height=img_size)
      
      remove(list=("p","local_healthy")) #remove map created and dataset to save memory
      
      #print all tweets labelled as "healthy" in the selected region##
      local_mislabelled <- local_selection[which(local_selection[,1] %in% explore$false_label),]
      
      p <- print_map(my_ggmap_local,"Mislabelled Tweets",my_coord=local_mislabelled,col="deeppink",pt_size=10)
      ggsave(file=filenames[6],width=img_size,height=img_size)
      
      remove(list=("p","local_mislabelled","my_ggmap_local")) #remove map created and dataset to save memory; also delete local google-map
    }  
...
#     my.plots <- vector(num.plots, mode='list')
    ...
    pdf(paste0('HexbinPlots_',tag,'.pdf'), onefile=TRUE)
...
#  histogram of longitude and latitude ----
  ...
  hist_coord <- function(dataframe, tag,explore){
    ...
    root <- 1/3
    root_tag <- as.character(root)
    
    ##create histogram of whole dataset##
    pdf(file=paste0("HistogramOfCoordinates_",tag,".pdf")
    for (i in 1:4){
      h <- hist(dataframe[,"longitude"], breaks = length(unique(dataframe[,"longitude"])),plot=F) #save histdata
      h$counts <- (h$counts)**root
      plot(h,xlab="longitude",ylab=paste0("(frequency)^",root_tag), main=filenames[(2*i)-1])
      #dev.off()
      remove(list =c("h"))
      
      h <- hist(dataframe[,"latitude"], breaks = length(unique(dataframe[,"latitude"])),plot=F) #save histdata
      h$counts <- (h$counts)**root
    ...
    dev.off()
   # png(filename=filenames[1])
    h <- hist(dataframe[,"longitude"], breaks = length(unique(dataframe[,"longitude"])),plot=F) #save histdata
   h$counts <- (h$counts)**root
   plot(h,xlab="longitude",ylab=paste0("(frequency)^",root_tag), main=paste0('All tweets  - ',tag,' - Longitude'))

    #dev.off()
    remove(list =c("h"))

    #png(filename=filenames[2])
    h <- hist(dataframe[,"latitude"], breaks = length(unique(dataframe[,"latitude"])),plot=F)
    h$counts <- (h$counts)**root
   ,xlab="latitude",main=paste0('All tweets - ',tag,' - Latitude'),
    #dev.off()
    remove(h)
  
    ##create histogram of sick subset##
    sub_set <- dataframe[dataframe[,5]==1,] #subset of all tweets labelled as "sick"
    #png(filename=filenames[3])
    h <- hist(sub_set[,"longitude"],xlab="longitude",main=paste0('Sick tweets  - ',tag,' - Longitude'), breaks = length(unique(sub_set[,"longitude"])))
    #dev.off()
    remove(h)
    
    #png(filename=filenames[4])
    h <- hist(sub_set[,"latitude"],xlab="latitude",main=paste0('sick tweets - ',tag,' - Latitude'), breaks = length(unique(sub_set[,"latitude"])))
    #dev.off()
    remove(list=c("h","sub_set")) #to save memory
    
    ##create histogram of sick subset##
    sub_set <- dataframe[dataframe[,5]==0,] #subset of all tweets labelled as "healthy"
    #png(filename=filenames[5])
    h <- hist(sub_set[,"longitude"],xlab="longitude",main=paste0('Healthy tweets  - ',tag,' - Longitude'), breaks = length(unique(sub_set[,"longitude"])))
    #dev.off()
    remove(h)        
    
    #png(filename=filenames[6])
    h <- hist(sub_set[,"latitude"],xlab="latitude",main=paste0('healthy tweets - ',tag,' - Latitude'), breaks = length(unique(sub_set[,"latitude"])))
    #dev.off()
    remove(list=c("h","sub_set")) #to save memory

           
    ##create histogram of mislabelled subset##
    sub_set <- dataframe[which(dataframe[,1] %in% explore$false_label),] #subset of all tweets labelled as "healthy"
    #png(filename=filenames[7])
    h <- hist(sub_set[,"longitude"],xlab="longitude",main=paste0('mislabelled tweets  - ',tag,' -Longitude'), breaks = length(unique(sub_set[,"longitude"])))
    #dev.off()
    remove(h)
    
    #png(filename=filenames[8])
    h <- hist(sub_set[,"latitude"],xlab="latitude",main=paste0('mislabelled tweets - ',tag,' - Latitude'), breaks = length(unique(sub_set[,"latitude"])))
    dev.off()
    remove(list=c("h","sub_set")) #to save memory
...
# plot histogram of User ID  ----
  ...
  user_activity <- function(dataframe,tag,title_plot="All tweets from "){
    user_ac <- as.data.frame(table(dataframe[,"userID"]))
    colnames(user_ac) <- c("userID","freq")
    pdf()
    ggplot(data =  user_ac, aes(x = users_ac$freq))+
