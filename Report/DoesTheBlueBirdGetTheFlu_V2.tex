...
Halfway through 2011, Twitter users sent approximately 200 million tweets per day or 6 billion tweets per month \citep{twitter_million_2011}. If we divide the latter number by the 151 million of Twitter users that were active on a monthly basis by the end of June 30th \citep{twitter_annual_2013}, we get a mean of 39.74 tweets per user per month. Making the conservative assumption that this number remains constant and does not increase, we would expect a mean of 1907.29 tweets per user over the whole four year period between March 3rd 2011 to March 4th 2015---much more than the 86 tweets sent by the most diligent Twitter user in the \texttt{sick\_users} data set.
...
In a first step, I removed all tweets before 2011-03-05 and after 2015-07-11 as well as outside the rough geographical window around the U.S. mainland (W $-125$°, W $-66$°, N $25$° , N $50$°) as described above, and aggregated the remaining 2'764'210'962 tweets and 15'229'049 users with regard to states and weeks in which the tweets where sent, \textit{i.e.} I calculated the number of tweets sent within a given week in a given state.
...
% 
% \prog{R} version: \textsf{sessionInfo()$R.version$version.string}
% 
% Base packages: \textsf{paste(sessionInfo()$basePkgs, collapse = ", ")}
% 
% Other packages: \textsf{paste(names(sessionInfo()$otherPkgs), collapse = ", ")}
\newpage\null\thispagestyle{empty}\cleardoublepage
