...
%\usepackage[monochrome]{xcolor} %for pure black white printings

\usepackage[authoryear,round,sectionbib]{natbib} %sectionbib option lets bibliography start right after text
...
% ======================================
% Define video float
\newfloat{video}{thp}{lop}[chapter]
\floatname{video}{Video}
\captionsetup[video]{margin=0pt}
% ======================================
...
\fancyfoot[ce, co]{\textrm{\name}}
\fancyfoot[re, lo]{\textrm{\versiondate}}
\renewcommand{\headrulewidth}{0.4pt}
\fancyhead[ce, co]{}
\fancyhead[re, lo]{\textrm{\grp}}
\fancyhead[le, ro]{\textrm{\inst}}
}
...
% =======================================
%create speical DESCRIPTION env
\usepackage{calc}
\makeatletter
...
\begin{description}
\item [Reliable data are available:] Most high-income countries have well-established Influenza sur\-veillance systems which provide reliable data sources that can be used as ``gold standard" with which the performance of new methods can easily be compared.
...
I believe that distinction given by \cite{goodman_what_2016} is the most straightforward and exhaustive categorisation of different forms of reproducibility. Even the concept of repeatability can be subsumed under this categorisation, namely as an especially stringent case of methods reproducibility. Most importantly, however, this categorisation makes it clear that methods, results, and inferential reproducibility can occur independently from each other, something that is often forgotten in the discussions about reproducibility. In particular, methods reproducibility can be fulfilled even though results and inferential reproducibility are absent---\textit{e.g.} if a finding based on Twitter user data can be perfectly recreated using the original raw data and codes, but fails replication when collecting data from a different set of Twitter users. The same holds true for results reproducibility, which can exist independently of methods reproducibility---\textit{e.g.} if the raw data and code of the original study are not provided, but replication is possible by recollecting the data using the methods described in the study---and inferential reproducibility---\textit{e.g.} if the original results can be replicated but still warrant a different interpretation because of theoretical or methodological discrepancies. Finally, inferential reproducibility can be fulfilled even in the absence of methods and results reproducibility---\textit{e.g.} when the raw data and code of the original study are missing, while the replication of the study based on the described methods shows an attenuated (or exacerbated) effect with the same directionality as the original study.
...
In Section~\ref{sec:comparison_cdc}, I will focus on results reproducibility by trying to analyse and compare the results from the Twitter classifier with the official flu data from the CDC. In Section~\ref{sec:methods_reproduce}, on the other hand, I will describe my attempts to reproduce two figures and one table from \cite{bodnar_data_2015} by using the processed data and code provided to me, \textit{i.e.} it contains my efforts to ensure methods reproducibility. Finally, Chapter~\ref{ch:discussion} contains further remarks on the methods and results reproducibility of the findings in \cite{bodnar_data_2015} as well as additional comments regarding inferential reproducibility.
...
l <- function(x){
  return(format(x,digits=20,big.mark= "'"))# propose to take "\text{'}"
}
...
Halfway through 2011, Twitter users sent approximately 200 million tweets per day or 6 billion tweets per month \citep{twitter_million_2011}. If we divide the latter number by the 151 million of Twitter users that were active on a monthly basis by the end of June 30th \citep{twitter_annual_2013}, we get a mean of 39.74 tweets per user per month. Making the conservative assumption that this number remains constant and does not increase, we would expect a mean of 1907.29 tweets per user over the whole four year period between March 3rd 2011 to March 4th 2015---much more than the \Sexpr{max(df_summary$ID_table_tot)} tweets sent by the most diligent Twitter user in the \texttt{sick\_users} data set.
...
\# of users  & 15'560'328 & \Sexpr{l(num_user_before)} & raw data set  \\
\cellcolor{grey}\# of users, pruned & \cellcolor{grey}-- & \cellcolor{grey}\Sexpr{l(num_user_after)} & \cellcolor{grey}pruned data set\\
\# of tweets  & 2'732'174'105 & \Sexpr{l(ntweets[1])} & raw data set  \\ 
\cellcolor{grey}\# of tweets  & \cellcolor{grey}-- & \cellcolor{grey}\Sexpr{l(ntweets_full_pruned)} & \cellcolor{grey}pruned data set \\
\cellcolor{grey}\# of sick users, 2011--2015 & \cellcolor{grey}182'801 & \cellcolor{grey}\Sexpr{l(tot_sick_user)} & \cellcolor{grey}pruned data set  \\
\cellcolor{grey}\# of users, 2011 & \cellcolor{grey}45'086 & \cellcolor{grey}\Sexpr{format(round(avg_tot_user_2011,0),digits=6,big.mark="'")} & \cellcolor{grey}pruned data set\\
mean tweet rate, 2011--2015 & 175.59 & \Sexpr{r2(mean_tweet_before)} & raw data set\\
\cellcolor{grey}mean tweet rate, 2011--2015 & \cellcolor{grey}-- & \cellcolor{grey}\Sexpr{r2(mean_tweet_after)} & \cellcolor{grey}pruned data set \\
\cellcolor{grey}mean tweet rate, weekly & \cellcolor{grey}-- & \cellcolor{grey}\Sexpr{r2(mean(avg_tw_tot))} & \cellcolor{grey}pruned data set\\
median tweet rate, 2011--2015 & 10 & \Sexpr{r2(median_tweet_before)} & raw data set\\
\cellcolor{grey}median tweet rate, 2011--2015 & \cellcolor{grey}-- & \cellcolor{grey}\Sexpr{r2(median_tweet_after)} & \cellcolor{grey}pruned data set\\
\cellcolor{grey}median tweet rate, weekly & \cellcolor{grey}-- & \cellcolor{grey}\Sexpr{r2(median(avg_tw_tot))} & \cellcolor{grey}pruned data set\\
max. \# of tweets per user, 2011--2015 &  1'119'384 & \Sexpr{l(max_tweet_before)} & raw data set\\
\cellcolor{grey}max. \# of tweets per user, 2011--2015 & \cellcolor{grey}-- & \cellcolor{grey}\Sexpr{l(max_tweet_after)} & \cellcolor{grey}pruned data set\\
min. \# of tweets per user, 2011--2015 & 1 & \Sexpr{l(min_tweet_before)} & raw data set\\
\cellcolor{grey}min. \# of tweets per user, 2011--2015 & \cellcolor{grey}-- & \cellcolor{grey}\Sexpr{l(min_tweet_after)} & \cellcolor{grey}pruned data set\\
\end{tabular}
\label{tab:data_set_diff}
\end{table}

In a first step, I removed all tweets before \Sexpr{time_window[[1]]} and after \Sexpr{time_window[[2]]} as well as outside the rough geographical window around the U.S. mainland (W $\Sexpr{coord_USA[1]}$°, W $\Sexpr{coord_USA[2]}$°, N $\Sexpr{coord_USA[3]}$° , N $\Sexpr{coord_USA[4]}$°) as described above, and aggregated the remaining \Sexpr{l(ntweets_full_pruned)} tweets and \Sexpr{l(num_user_after)} users with regard to states and weeks in which the tweets where sent, \textit{i.e.} I calculated the number of tweets sent within a given week in a given state.
...
When comparing the relative number of tweets labelled as ``healthy" with those labelled as ``sick", we can see slight differences in the distribution, which become even more accentuated when normalising with the total number of tweets per state (Figure~\ref{fig:tweets_state_healthy_sick}). However, the states with the most pronounced differences (District of Columbia, Montana, South Dakota, North Carolina) are almost all states or districts, respectively, with a very low overall tweet count (North Carolina being the exception). A Chi-squared test for independence between the two distributions gives a $p$-value of \Sexpr{r4(chi_sick_healthy_state$p.value)}. 
...
\begin{DESCRIPTION}
\item [Region 1: ] Connecticut, Maine, Massachusetts, New Hampshire, Rhode island, Vermont;
\item [Region 2: ] New Jersey, New York, Puerto Rico, the U.S. Virgin Islands;
\item [Region 3: ] Delaware, District of Columbia, Maryland, Pennsylvania, Virginia, West Virginia;
\item [Region 4: ] Alabama, Florida, Georgia, Kentucky, Mississippi, North Carolina, South Carolina, Tennessee;
\item [Region 5: ] Illinois, Indiana, Michigan, Minnesota, Ohio, Wisconsin;
\item [Region 6: ] Arkansas, Louisiana, New Mexico, Oklahoma, Texas;
\item [Region 7: ] Iowa, Kansas, Missouri, and Nebraska;
\item [Region 8: ] Colorado, Montana, North Dakota, South Dakota, Utah, Wyoming;
\item [Region 9: ] Arizona, California, Hawaii, and Nevada;
\item [Region 10:] Alaska, Idaho, Oregon, and Washington.
\end{DESCRIPTION}
...
\centering
  \caption{Difference between the CDC ILI activity levels and those calculated based on the relative amount of sick Twitter users in each corresponding state and week. White means that CDC and Twitter activity levels are exactly the same, red means that the CDC reported higher ILI activity levels in a given state and week than those calculated from the Twitter data set, blue indicates the opposite. The PDF-version of this thesis contains a playable video.}
    \label{vid:cdc_twitter_comp_state}
\end{video}
...
\centering
  \caption{Difference between the official ILI activity levels provided by the state of New Jersey and those calculated based on the relative amount of sick Twitter users in each corresponding week and county of New Jersey. White means that the official and Twitter activity levels are exactly the same, red means that the state of New Jersey reported higher ILI activity levels in a given county and week than those calculated from the Twitter data set, blue indicates the opposite. The PDF-version of this thesis contains a playable video.}
    \label{vid:cdc_twitter_comp_jersey}
\end{video}
...
\centering
  \caption{Difference between the official ILI activity levels provided by the state of New Jersey and those calculated based on the relative amount of sick Twitter users in each corresponding week and county of Oregon. White means that the official and Twitter activity levels are exactly the same, red means that the state of Oregon reported higher ILI activity levels in a given county and week than those calculated from the Twitter data set, blue indicates the opposite. The PDF-version of this thesis contains a playable video.}
    \label{vid:cdc_twitter_comp_oregon}
\end{video}
...
Therefore, I went on to fit the very same SIR model that is shown in Figure~\ref{fig:cdc_fit_bodnar_thesis_SIR}. To do so, I needed to know which data the model was built on. I extracted the three starting coordinates of the yearly curves shown in the figure and compared them with the data provided to me by Todd. It turned out that the starting coordinates of the curve only matched the values from the full retrospective Twitter model (\textit{i.e.} retrospective AR(2) model based on the CDC data combined with the retrospective Twitter base model), so I built my second SIR model based on these values. As can be seen from Figure~\ref{fig:SIR_comparison_full}, the resulting yearly and combined ILI curves are virtually indistinguishable from the ones shown in \cite{bodnar_data_2015}.

It is very peculiar, however, that the model parameters which I calculated for the two replicated SIR models described above do not match the yearly and combined values for $\gamma$ and $\beta$ presented in \cite{bodnar_data_2015}---even though the model curves seem to match. As can be seen from Table~\ref{tab:nationalparams_replicated}, the optimal yearly and combined values of $\gamma$ and $\beta$ deviate considerably from the ones depicted in Table~\ref{tab:nationalparams}, \textit{i.e.} the values reported in \cite{bodnar_data_2015}. This is true for the SIR model based on the CDC data as well as for the SIR model based on the full retrospective model data. At the same time, however, the curves that result when creating an SIR model based on the values in Table~\ref{tab:nationalparams_replicated} and the \texttt{full\_both2} data provided to me are virtually indistinguishable from the curves depicted in \cite{bodnar_data_2015}, \textit{i.e.} in Figure~\ref{fig:cdc_fit_bodnar_thesis_SIR}. This implies that the $\beta$ and $\gamma$ values reported in \cite{bodnar_data_2015} are not the ones that were used to create Figure~\ref{fig:cdc_fit_bodnar_thesis_SIR}.
...
\centering
  ...
  \caption{}
  \end{subfigure}
  \caption{SIR model curves using yearly (a) and combined (b) parameters to model the ILI prevalence throughout one year. Red indicates the official ILI rates from the CDC, while cyan and blue show the SIR model curves based on the parameters reported in \cite{bodnar_data_2015} and the ones recalculated by me, respectively, based on the very same data (\textit{i.e.} the \texttt{full\_both2} values).}
...
However, Twitter data can still be of use, especially when it comes to now-cast ILI rates, \textit{i.e.} to assess the current prevalence of influenza-like symptoms based on the preliminary CDC data that has been published \citep{paul_worldwide_2015}. This can also be seen in Figures~\ref{fig:TwitterModel_comparison_ar} and ~\ref{fig:TwitterModel_comparison_base}, which show that the the CDC AR(2) combined with the Twitter baseline model provide slightly better results than the AR(2) model alone, respectively. 
...
\begin{description}
\item [Faulty data handling:] Errors in the aggregation and processing of the Twitter data led to different results.
\item [Differences in the data set:] The findings from \cite{bodnar_data_2015} are based on Twitter data that are different from the data I analysed.
\item [Classification errors:] There are errors present in the Twitter classifier code that led to wrongly labelled tweets.
\item [Additional modelling needed:] The output from the Twitter classifier alone is not sufficient to make ILI predictions.
\end{description}
...
\subsection{Faulty data handling}
...
\subsection{Differences in the data set}
...
\subsection{Additional modelling needed}
...
\thispagestyle{empty}\markboth{Bibliography}{Bibliography}
\setlength{\bibsep}{0pt}
{
\renewcommand{\baselinestretch}{1.15}\normalfont % precise values needs to be determined.
\renewcommand{\bibname}{}
\bibliographystyle{mywiley}
\bibliography{Masterarbeit.bib}
%\defbibheading{bibliography}[\refname]{} %removes heading over bibliography
}
...
\appendix
\chapter{Appendices} 
\addcontentsline{toc}{chapter}{\bfseries{Appendices}}
\thispagestyle{empty}\markboth{Appendices}{Appendices}
\section{R-Packages} 
\addcontentsline{toc}{section}{R-Packages}
\thispagestyle{empty}\markboth{R-Packages}{R-Packages}
\label{app:packages}
...
require("chron")
require("classInt")
require("compiler", lib.loc="/Library/Frameworks/R.framework/Versions/3.2/Resources/library")
require("devtools")
require("doParallel", lib.loc="~/Library/R/3.2/library")
require("feather")
require("foreach", lib.loc="~/Library/R/3.2/library")
require("geosphere", lib.loc="~/Library/R/3.2/library")
require("gganimate")
require("ggdendro")
require("ggmap", lib.loc="~/Library/R/3.2/library")
require("ggplot2")
require("grid")
require("gridBase")
require("gridExtra")
...
require("lattice")
require("lubridate")
require("maps", lib.loc="~/Library/R/3.2/library")
require("maptools")
require("mgcv") #for in.out()
require("pageviews")
require("parallel", lib.loc="/Library/Frameworks/R.framework/Versions/3.2/Resources/library")
require("pdftools")
require("plotrix")
require("plyr", lib.loc="~/Library/R/3.2/library")
require("png")
require("poweRlaw", lib.loc="~/Library/R/3.2/library")
require("profvis")
require("qdap")
require("raster")
require("rbenchmark")
...
require("rworldmap")
require("rworldxtra")
require("scales") # for function alpha() & colour_ramp()
require("stringr")
require("tabulizer")
require("timezone")
require("TSclust")
...
\thispagestyle{empty}\markboth{Github}{Github}
\label{app:github}
...
\begin{description}
  \item [DataCleansing:] Contains all code files needed for aggregating, transforming and pruning the raw output from the Twitter classifier. It also contains the processed data sets. 
  \item [StatisticalAnalysis:] Contains all code files needed to perform the statistical analyses of the data sets and the comparisons with the CDC data. It also contains the corresponding figures and output summaries.
   \item [PhDThesisBodnar:] Contains all information revolving around Todd Bodnar's PhD thesis, including a summary of his communication to me. it also includes the code files and corresponding results of my reproduction attempts.
   \item [TwitterParse:] Contains all information provided to me by Todd Bodnar with regard to the Java-based Twitter classifier, as well as my own additions.
  \item [TestingCode:] Contains a collection of code files which just served for ``quick-and-dirty" testing and will most likely not work without some tweaking. They are just included for completeness' sake. and to satisfy the especially curious ones. None of the output is directly used for this report, yet some of the code elements have found their way into the final code version provided in the other folder.
  \item [Report:] Contains all necessary files to reproduce this Master thesis.
\end{description}
