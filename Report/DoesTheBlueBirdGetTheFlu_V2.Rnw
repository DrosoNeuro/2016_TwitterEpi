...
\text{tweet rate} = \frac{T_{\text{tot}}}{U_\text{tot}} = \frac{T_{\text{geotagged}} \cdot \frac{1}{0.0085}}{U_\text{geotagged} \cdot \frac{1}{0.031}} \label{eq:1}
  \end{align}
%$$\text{tweet rate} = \frac{T_{\text{tot}}}{U_\text{tot}} = \frac{T_{\text{geotagged}} \cdot \frac{1}{0.0085}}{U_\text{geotagged} \cdot \frac{1}{0.031}}$$% = \frac{T_{\text{sick\_users}} \cdot \frac{T_{\text{all\_tweets}}}{T_{\text{sick\_users}}} \cdot \frac{1}{0.0085}}{U_\text{sick\_users} \cdot \frac{U_{\text{all\_tweets}}}{U_{\text{sick\_users}}} \cdot \frac{1}{0.031}}$$
...
Finally, by applying Equation~\ref{eq:1} described in Section~\ref{sec:sick_user_exploratory}, we can estimate the adjusted mean (\Sexpr{r2(mean(avg_tw_tot/(0.0085/0.031)))}) and median \Sexpr{r2(median(avg_tw_tot/(0.0085/0.031)))} of the twitter rate of the whole Twitter population, as well the mean tweet rate for sick (\Sexpr{r2(mean(avg_tw_sick/(0.0085/0.031)))}) and healthy (\Sexpr{r2(mean(avg_tw_healthy/(0.0085/0.031)))}) supbopulation, respectively.\newpage
...
A Kolmogorov-Smirnov test reveals that the weekly activity of the tweets labelled as ``sick" is in fact significantly different from the weekly activity of the tweets labelled as ``healthy" ($p$ = \Sexpr{r4(ks_sick_healthy_seasonal_abs$p.value)} and $p$ = \Sexpr{r2(ks_sick_healthy_seasonal_rel$p.value)} for the uncorrected and corrected weekly tweet counts, respectively). See Figure~\ref{fig:tweets_seasonal_healthy_sick} for a side-by-side comparison of both the uncorrected and corrected weekly tweet activity.
...
\centering
  ...
  \end{subfigure}
  \caption{Comparison between the full retrospective Twitter model depicted in \cite{bodnar_data_2015} (a) and replications consisting of a simple retrospective AR(2) model based only on CDC ILI rates (b) and the retrospective Twitter base model (c)}
...
In a second part, I will then put emphasis on the discussion of the different issues of reproducibiliy that I have outlined in Chapter~\ref{ch:reproducibility} and that have arisen during the course of the of this work. I will also make suggestions about how to overcome such issues in the future. 
