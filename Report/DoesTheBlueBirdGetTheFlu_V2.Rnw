...
Repeating the calculations using number of Twitter users instead of number of tweets yields similar results (see Figures~\ref{fig:tweets_state_full_user}, \ref{fig:tweets_state_full_scatter_user}, \ref{fig:tweets_state_full_scatter_user_log}, and \ref{fig:tweets_state_healthy_sick_user}; $p$ = \Sexpr{r4(chi_sick_healthy_state_user$p.value)}).\newpage
...
\centering
  ...
  \caption{}
  \end{subfigure}
  \caption{Histograms of numbers of the tweets sent per week during the 208 weeks between \Sexpr{time_window[[1]]} and \Sexpr{time_window[[2]]} (bin size = 1 week). (a) shows only tweets ``sick"; (b) shows only tweets labelled as ``healthy" by the classifier. The grey bars indicate the relative number of sick or healthy tweets sent per week without normalisation. The black lines indicate the relative number of sick or healthy tweets sent per week normalised by the total amount of tweets sent in that week.}
