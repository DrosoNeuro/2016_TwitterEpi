...
\begin{itemize}
  \item \texttt{all\_tweets} contains the whole set of rated tweets with \Sexpr{l(ntweets[1])}~tweets and )
  \item \texttt{one\_hundred} contains the rated tweets of those users who sent at least 100 tweets (\Sexpr{l(ntweets[2])}~rows)
  \item \texttt{sick\_users} contains the rated tweets of all those users who sent at least one sick tweet (\Sexpr{l(ntweets[3])}~rows)
\end{itemize}
...
I ignored the \texttt{one\_hundred} data set and only analysed the other two sets. All data sets were handled using \citeppackages{datatable} or \citeppackages{bigmemory}.
...
\caption{The total number of users who only sent tweets labelled as ``sick" (\Sexpr{df_summary$num_only_sick_users}), users who sent at least one tweet labelled as ``sick" and ``healthy" (\Sexpr{df_summary$num_both}), and users who only sent tweets labelled as ``healthy" (\Sexpr{df_summary$num_only_health_users})}
...
If we make the naïve assumption that geotagged tweets represent a subset of all tweets that is uniformly distributed over all Twitter users (in other words: it is assumed that each Twitter user adds geolocation to 0.85 \% of his tweets) we receive a mean tweet rate (\Sexpr{r2(df_summary$num_all_tweets/df_summary$num_all_users/0.0085)} tweets per user in the \texttt{sick\_users} data set) that is comparable to the mean tweet rate of all Twitter uers (1907.29 tweets per user).
...
$$\text{tweet rate} = \frac{T_{\text{tot}}}{U_\text{tot}} = \frac{T_{\text{all\_tweets}} \cdot \frac{1}{0.0085}}{U_\text{all\_tweets} \cdot \frac{1}{0.031}} = \frac{T_{\text{sick\_users}} \cdot \frac{T_{\text{all\_tweets}}}{T_{\text{sick\_users}}} \cdot \frac{1}{0.0085}}{U_\text{sick\_users} \cdot \frac{U_{\text{all\_tweets}}}{U_{\text{sick\_users}}} \cdot \frac{1}{0.031}}$$

Here, $T_\text{tot}$, $T_\text{all\_tweets}$, and $T_\text{sick\_users}$ depcict the number of tweets in the real world, the \texttt{all\_tweets} data set, and the \texttt{sick\_users} data set, respectively, wheras $U_\text{tot}$, $U_\text{all\_tweets}$, and $U_\text{sick\_users}$ denominate the number of Twitter users in the real world, the \texttt{all\_tweets} data set, and the \texttt{sick\_users} data set, respectively. 

Applying this formula yields a mean tweet rate of xy tweets per users between March $3^\textit{rd}$ 2011 to March $4^\textit{th}$ 2015, which is considerably lower than our conservative estimate using the official Twitter data. Hence, I am led to believe that the \texttt{sick\_users} data set does is neither representative of rest of the data set, let alone the total corpus of tweets produced in the real world. This should not come as a suprise, since the \texttt{sick\_users} data set is supposed to contain the tweets of all those users who sent at least one tweet that was classified as ``sick" (Bodnar, personal communication).
...
\centering
  ...
  \caption{}
  \end{subfigure}
  \caption{Histogram of numbers of tweets sent per user during the 208 weeks between \Sexpr{time_window[[1]]} and \Sexpr{time_window[[2]]} (bin size = 1). (a) contains only tweets from users who sent at least one tweet labelled as ``sick" and one tweet labelled as ``healthy".  Mean = \Sexpr{avg_med_both[[1]]} (solid line); median = \Sexpr{avg_med_both[[2]]} (dashed line) (b) contains only tweets from users who never sent a tweet that was labelled as ``sick" by the classifier.  Mean = \Sexpr{avg_med_healthy[[1]]} (solid line); median = \Sexpr{avg_med_healthy[[2]]} (dashed line). As can be seen, mode, median and mean of the number of tweets sent per user are significantly lower in (a) than in (b). Also, note that by construction (a) does not contain any user who only sent one tweet (since the users in this group are defined by having sent at last one tweet labelled ``sick" and one tweet labelled as ``healthy")}
...
@
The complete data set consisted of a total of \Sexpr{l(ntweets[1])} tweets and hence was larger than the set reported by \citep{bodnar_data_2015} which contained 2'732'174'105 tweets. This difference is simply due to the fac the tweets in my data set were collecte until July 2015, while the tweets analysed in \citep{bodnar_data_2015} were only collected up to March 2015.

In a first step, I removed all tweets before \Sexpr{time_window[[1]]} and after \Sexpr{time_window[[2]]} as well as outside the rough geographical window around the US mainland (W $\Sexpr{coord_USA[1]}$°, W $\Sexpr{coord_USA[2]}$°, N $\Sexpr{coord_USA[3]}$° , N $\Sexpr{coord_USA[4]}$° ) as described above, and aggregated the remaining \Sexpr{l(ntweets_full_pruned)} tweets and xy users with regard to states and weeks in which the tweets where sent, i. e. I calculated the number of tweets sent within a given week in a given state
...
There were a total amount of \Sexpr{l(sum(df_processed$sick))} tweets labelled as ``sick", a number that is considerably larger than the \Sexpr{l(df_summary$num_sick_tweets)} tweets labelled as ``sick" in the \texttt{sick\_users} data set. This further shows that that the latter does not contain the full subset of tweets labelled as ``sick". Relatively speaking, \Sexpr{r2(sum(df_processed$sick)/sum(df_processed$total)*100)}~\% of all tweets in the \texttt{all\_tweets} data set were labelled as ``sick" (as opposed to \Sexpr{r2(sum(df_summary$prop_all[,1])*100)}~\% in the \texttt{sick\_users} data set).
...
What is peculiar, however, is the fact that the total number of sick users found in the Twitter data set is considerably lower than the number reported in \cite{bodnar_data_2015} {\Sexpr{l(tot_sick_user)} vs 182'801 users, respectively), despite the former being an upper estimate of the total number of individual sick users. On the other hand, the number of individual users found in the data set during the first year (2011) for the whole country is larger than the one reported by \cite{bodnar_data_2015}: \Sexpr{l(avg_tot_user_2011)} and 45'086, respectively.

\textbf{Using the information that only around 0.85~\% of all tweets are geotagged \citep{sloan2013knowing} and 3.1~\% of users send geotagged tweets \citep{sloan2013knowing}, we can estimate the total number of Twitter users active in 2011 based on the two mentioned sample estimates, respectively. While the former sample estimate gives us an average of \Sexpr{avg_tot_user_2011/0.85*100} active users in 2011, the latter estimate based on \cite{bodnar_data_2015} only amounts to \Sexpr{45086/0.85*100} total active users in 2011 --- a number which is a far cry from the roughly 19 (March 2011) to 30 (December 2011) million of monthly active twitter users officially reported by Twitter in 2011 \citep{twitter_annual_2013}.}

In addition, I could observe a very peculiar difference in the average tweet frequency between healthy and sick users. While healthy users sent an average of \Sexpr{r2(mean(avg_tw_healthy))} tweets per week, sick users sent an average \Sexpr{r2(mean(avg_tw_sick))} tweets per week (see Figure~\ref{fig:avg_tw_diff}), a difference that is highly significant (Mann-Whitney U-Test, $p$ =\Sexpr{r2(diff_wilcox_avg_tw$p.value)}). Also, the mean tweet rate of all users combined (\Sexpr{r2(mean(avg_tw_tot))}) is six times smaller than the mean tweet rate per user reported in \cite{bodnar_data_2015} (175.59). The median (\Sexpr{r2(median(avg_tw_tot))}) tweet rate, however, is considerably higher than the median tweet rate reported in \cite{bodnar_data_2015} (10).

These points give additional reason to believe that the data reported in \cite{bodnar_data_2015} are not the same as the data I was analysing for this thesis.
...
\hfill
  ...
  \caption{}
  \end{subfigure}
  \caption{Histograms of numbers of the tweets sent per week during the 208 weeks between \Sexpr{time_window[[1]]} and \Sexpr{time_window[[2]]} (bin size = 1 week). (a) and (c) contain only tweets labelled as ``healthy"; (b) and (d) contain only tweets labelled as ``sick" by the classifier. The lower two histograms were normalised by the total amount of tweets sent per week. As can be seen, the tweets labelled as ``sick" follow a markedly different temporal pattern.}
...
\caption{Relative number of tweets sent per state (a) and Twitter users (b) in the \texttt{all\_tweets} data set between \Sexpr{time_window[[1]]} and \Sexpr{time_window[[2]]} plotted against each state's relative population size}
\end{figure}
...
\hfill
  ...
  \caption{}
  \end{subfigure}
  \caption{Histograms of numbers of the tweets sent in each state during the 208 weeks between \Sexpr{time_window[[1]]} and \Sexpr{time_window[[2]]} (bin size = 1 week). (a) and (c) contain only tweets labelled as ``healthy"; (b) and (d) contain only tweets labelled as ``sick" by the classifier. The lower two histograms were normalised by the total amount of tweets sent per state As can be seen, the tweets labelled as ``sick" follow a somewhat different spatial pattern.}
...
\hfill
  ...
  \caption{}
  \end{subfigure}
  \caption{Histograms of number of users active in each state during the 208 weeks between \Sexpr{time_window[[1]]} and \Sexpr{time_window[[2]]} (bin size = 1 week). (a) and (c) contain only tweets labelled as ``healthy"; (b) and (d) contain only users with at least one tweet labelled as ``sick" by the classifier. The lower two histograms were normalised by the total number of user active per state. As can be seen, the users who were ``diagnosed" as ``sick" at some point by the classifier, follow almost the same spatial pattern.}
