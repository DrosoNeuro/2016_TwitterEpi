...
In order to assess the validity of the ILI predictions provided by the flu classifier, I compared the normalised number of tweets labelled as "1 = sick" per week with the official ILI reports from the CDC on the national, regional and state level(extracted using the "cdcfluview" package \citeppackages{cdcfluview}).\newline
...
\subsection{The findings can only be replicated by using the classified tweets as a starting point for more intricate models}
...
\newpage
\bibliographystylepackages{unsrtnat}
